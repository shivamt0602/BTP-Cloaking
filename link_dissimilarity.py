...
try:
    ...
    with open('apple.com_twisted.csv', 'r') as csvfile:
        ...
        for row in csv_reader:
            ...
            try:
                ...
                response_googlebot = requests.get(url, headers={'User-Agent': user_agents['googleua']})
                    ...
                    if bot_links:
                        for link in bot_links:          
                            if link:
                                href = link.get('href')
                                if href:
                                    bot_hrefs.append(href) 

                num_hrefs_googlebot = len(bot_hrefs)                    
                ...
                with open('href_counts.txt', 'a') as file:
                    file.write(f"URL: {url}\n")
                    file.write(f"Number of HREFs (Googlebot): {num_hrefs_googlebot}\n")
