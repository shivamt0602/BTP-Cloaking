...
def compare_html_files(file1_path, file2_path, encoding='utf-8'):
    with open(file1_path, 'r', encoding=encoding, errors='replace') as file1, open(file2_path, 'r', encoding=encoding, errors='replace') as file2:
...
csv_file_path = 'comparison_results_html.csv'
...
for filename in os.listdir(bot_txt_folder):
    ...
    if os.path.isfile(bot_file_path) and os.path.isfile(normal_file_path):
        ...
        results.append({'url': url, 'html_result': result})

# Write results to CSV file
with open(csv_file_path, 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['url', 'html_result']
